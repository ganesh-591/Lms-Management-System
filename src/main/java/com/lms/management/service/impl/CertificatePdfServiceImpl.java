    ...
    public String generatePdf(Certificate certificate,
        ...
        try {
            ...
            String template = new String(
                    Files.readAllBytes(Paths.get("src/main/resources/templates/certificate-template.html"))
            );

            String verifyUrl = "http://localhost:5151/api/certificates/verify?token="
            ...
            template = template.replace("{{examTitle}}", examTitle);
            template = template.replace("{{score}}", String.valueOf(certificate.getScore()));
            template = template.replace("{{date}}",
                    certificate.getIssuedDate()
                            .format(DateTimeFormatter.ofPattern("dd MMM yyyy")));
            template = template.replace("{{certificateId}}", certificate.getCertificateId());
